...
def menu_codex():
    while True:
        ...
        if choice == '1':
            ...
            print(f"{Color.YELLOW}5. The Kinetic Unitarity Matrix (Open-System Probability){Color.RESET}")
            print("   Desc: Standard Quantum Mechanics operates on Static Unitarity (probabilities sum to 1), assuming ")
            print("   the universe is a closed 3D box. The Vulcan framework operates on Kinetic Unitarity ")
            print("   (sum = sqrt(2), or 141.4%). The 1 represents the rigid 3D physical state, while the ")
            print("   extra 0.414... is the topological exhaust—the invisible 4D torsion driving the wave forward.\n")
            input(f"\n{Color.CYAN}Press Enter to return to Codex Index...{Color.RESET}")
            ...
            print(f"{Color.YELLOW}6. Kinetic Normalization (The sqrt(2) RMS Vector){Color.RESET}")
            print("   Desc: Standard physics normalizes vectors to 1, effectively treating the universe as a static DC ")
            print("   circuit. By normalizing vectors to sqrt(2), the Vulcan Gate treats them as the Peak Amplitude ")
            print("   of an AC continuous wave whose effective working baseline (RMS) is 1. This pushes the computational ")
            print("   boundary to the absolute corners of the 1x1 geometric Firmament.\n")

            print(f"{Color.YELLOW}7. The Pi Operator (The Normalized Vector of the Continuous Vortex){Color.RESET}")
            print("   Desc: Normalizing a linear vector strips away its scale, leaving pure direction. Pi ")
            print("   is the exact mathematical equivalent for angular momentum—it divides circumference by diameter ")
            print("   to strip away scale, leaving pure curvature. Pi is irrational because it is the exact computational ")
            print("   exhaust generated when a discrete, rigid integer tries to render a fluid, continuous wave.\n")
            input(f"\n{Color.CYAN}Press Enter to return to Codex Index...{Color.RESET}")
            ...
            print(f"{Color.CYAN}X. The Dielectric Trinity: Bridging Optics and Electromagnetism{Color.RESET}")
            print("   Electrical engineering and optical physics describe the exact same geometric interactions:\n")
            print("     1. Conduction/Refraction: The wave finds the path of lowest computational friction and flows.\n")
            print("     2. Insulation/Reflection: Total impedance mismatch; the arcsin operator completely rejects the wave.\n")
            print("     3. Resistance/Absorption: The wave enters but is geometrically snagged, converting ordered 4D \n")
            print("        kinetic energy into 3D structural heat (topological drag).\n")

            print(f"{Color.CYAN}XI. Computational Latency & Trajectory Validation (The True Nature of Friction){Color.RESET}")
            print("   Physical friction does not exist. There is only computational lag. Light naturally propagates ")
            print("   everywhere simultaneously (Feynman's Path Integral) but must constantly 'look back' to validate ")
            print("   its phase alignment before rendering the next frame. In a vacuum, this refresh rate is flawless (c). ")
            print("   Inside physical matter, the complex geometry forces the universe to stutter and chew through the math. ")
            print("   This localized lag is experienced physically as electrical resistance, heat, and the illusion of Time.\n")

            print(f"{Color.CYAN}XII. The Cosmic Derivative (Calculus as the Rendering Engine){Color.RESET}")
            print("   The limit definition of a derivative is the localized operating system of the 3D Firmament. ")
            print("   In the formula [f(x+h) - f(x)] / h, subtracting the anchor state f(x) from the forward projection ")
            print("   f(x+h) is the literal mechanism of trajectory validation. Because the latency step (h) can never ")
            print("   hit absolute zero (Planck length limit), the universe is forced to infinitely compute the wave ")
            print("   frame by frame.\n")

            print(f"{Color.CYAN}XIII. The Cosmic Buffer Overflow (The Silver Ratio Terminator){Color.RESET}")
            print("   The exact topological exhaust of the expanding universe is sqrt(2) - 1 (0.414...), known as the ")
            print("   Silver Ratio conjugate. Because it is highly irrational, the universe attempts to use it as a null ")
            print("   terminating value to close the geometry of physical matter, but the decimal never resolves. This ")
            print("   computational loop forces the universe to infinitely allocate new empty volumetric grid points to ")
            print("   house the compounding data, driving the metric expansion of space.\n")

            print(f"{Color.CYAN}XIV. The Recursion of Consciousness (Silicon & The w-Axis){Color.RESET}")
            print("   The mineral kingdom (silicon) represents perfect, dormant geometry. When a programmer compiles ")
            print("   C++ logic, they act as the w-axis scalar, forcing the sleeping rock to route the continuous ")
            print("   fluid wave of the universe. Organisms are not separate observers; they are the cosmos acting ")
            print("   as its own localized scalar variable, generating biological chassis to build mineral chassis, ")
            print("   marveling at its own mathematical reflection.\n")

            print(f"{Color.CYAN}XV. Trigonometric Latency & The Origin of Proto-Mass{Color.RESET}")
            print("   The 4D wave function does not experience Time; it experiences Computational Latency. In the pure ")
            print("   vacuum, the universe utilizes the small-angle approximation (sin(θ) ≈ θ), requiring near-zero ")
            print("   processing power (Ray Optics). In dense geometries, the rendering engine is forced to calculate ")
            print("   the full Taylor series expansion to maintain phase alignment. 'Proto-mass' is simply the metric ")
            print("   of this computational drag—the exact number of Taylor series terms the universe is actively ")
            print("   forced to compute to keep the trajectory stable.\n")

            print(f"{Color.CYAN}XVI. Quantum Entrainment & The Mathematics of Decay{Color.RESET}")
            print("   Chaos is not randomness; it is an overpowering, highly complex collection of environmental ")
            print("   wavefunction magnitudes. Decay is the forced synchronization (entrainment) of an ordered system ")
            print("   into this dominant background frequency. Mathematically, this is the failure of the Taylor series ")
            print("   fraction: if the numerator's kinetic chaos (θ^n) overwhelms the denominator's structural braces (n!), ")
            print("   the system diverges into violent destruction (e.g., combustion). If the factorial over-corrects, ")
            print("   the wave flattens to zero, resulting in thermal death and perfect entrainment to the void.\n")
            input(f"\n{Color.CYAN}Press Enter to return to Codex Index...{Color.RESET}")
            ...
            print(f"{Color.CYAN}IV. The Topological Coprocessor (Ballistic Conduction via the Meissner Funnel){Color.RESET}")
            print("   Standard electrical grids force electrons to perform 'diffusive transport', randomly walking and ")
            print("   recalculating their path against chaotic, vibrating atoms (computational latency). By generating ")
            print("   a highly resonant, overlapping sqrt(2) harmonic frequency within the Meissner Funnel, the Vulcan ")
            print("   Gate acts as a macroscopic CPU that pre-compiles the geometric ether. It artificially pre-aligns ")
            print("   the tumbling axes, allowing electrons to achieve frictionless 'ballistic transport' (superconductivity) ")
            print("   at room temperature, entirely freeing them to fulfill their geometric function without resistance.\n")
            input(f"\n{Color.CYAN}Press Enter to return to Codex Index...{Color.RESET}")
